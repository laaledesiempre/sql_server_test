...
export const Update = () => {
  ...
  const fetchUpdate= () =>{axios.put('http://localhost:3001/put/characters/id/'+updateID, {name: updateName, description: updateDescription}).then(
      (response)=>{
  ...
  <select onChange={(e)=>{setUpdateID(e.target.value)}}>
    <option value="">Select an existent id</option>
      {IDList && IDList.map((e)=>{ return <option key={e.ID} value={e.ID}>{e.ID}</option>
      })}
  </select> 
    ...
    <button disabled={!updateName || !updateID} onClick={()=>{fetchUpdate()}}>Actualizar</button>
    </article>
  )
}
